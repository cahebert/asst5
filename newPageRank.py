...
#scc = StremaingContext(conf=conf)
#scc.checkpoint("_checpoint")
...
for iteration in range(iters):
    ...
    if (iteration % 20 == 0):
        ...
        contribution2.checkpoint()
        ranks.checkpoint()
    print(iteration)

#swappedRanks = ranks.map(lambda x:(x[1], x[0]))
#sortedRanks = swappedRanks.sortByKey(ascending =False)
#r_top = np.asarray(sortedRanks.collect().take(5))

rank_sorted = ranks.sortBy(lambda pair:pair[1])
